...
def change_connection_status(connectionStatus):
    ensure_session()
    response = SESSION.post(create_absolute_url("/data/Connect.json"), params=PARAMS,
                            data={'req_connect': connectionStatus, 'csrf_token': CSRF_TOKEN})
...
def ensure_session():
    ...
    password = (sha256(str(challenge+":"+str(DEVICE_PASSWORD)).encode('utf-8')).hexdigest())
    data = {'csrf_token': CSRF_TOKEN, 'showpw': '0', 'challengev': challenge, 'password': password}
    response = SESSION.post(create_absolute_url("/data/Login.json"), params=PARAMS, data=data)
    SESSION.cookies.set("challengev", challenge, domain='speedport.ip')
    return is_logged_in()

def extract_variable(json, key):
    return re.match("\"varid\"\\s*:\\s*\"" + key + "\",\\s*\"varvalue\"\\s*:\\s*\"([^\"]+)", json)
...
def update_csrf():
    if is_logged_in():
        response = SESSION.get(
            create_absolute_url("/html/content/internet/connection.html?lang=de"))
        global CSRF_TOKEN
